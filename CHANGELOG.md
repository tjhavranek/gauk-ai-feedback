...
- On a computer, the question above the switch ("What are you submitting")
  stands above it, and the rounded frame fits only the three choices. The
  frame used to run the full width of the page with the question on a row of
  its own inside it. The phone layout is unchanged.
