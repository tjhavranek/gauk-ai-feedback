<!-- GENERATED by tools/build.py. DO NOT EDIT THIS FILE.
     ...
     sources INDEX:f5ca217dc72b body_cs:e5b4b184fae0 body_en:758f426b838d build:862830c2923f criteria:ede5f6a5b111 round:cbb137cf5fc9 -->

# GA UK pre-submission review: the prompt

Unofficial. On GitHub, click the copy icon in the top right corner of the grey box below; it copies everything from `=== PROMPT BEGIN ===` to `=== PROMPT END ===`. Paste it into a new chat, then paste your draft after it. The [README](../README.md) explains how to use it and what it will not do.
...
Character counts include spaces. You do not count them yourself; see the formal findings block.
  ...
  Characteristics of the team: 50-750 characters
      Describe the team members who are paid from personnel costs: for
      each, the leader included, the department and their part in the
      project, and for a student the year of study. Do not name anyone
      who is not in the team table.
...
BUDGET RULES
  ...
  - The outlook for later years is entered with overhead included. At
    continuation it can be raised by at most 10 percent, so overhead
    left out of the outlook cannot be fully added later.
  - Funds are granted for one calendar year and cannot be carried over
    to the next, so a trip or conference planned for a later year
    belongs in that year's budget.
...
REMINDERS BEFORE SUBMITTING
  ...
  - Timetable: every year of the project runs from January to
    December, so a one-year project runs from January to December
    2027.
  - The team characteristics field in the form (not the team table):
    for each member, the leader included, give the department and
    their part in the project, and for a student the year of study.
    People who are not in the team table belong only in the proposal,
    with a word on how they are paid, if at all.
  - Budget justification: the amounts match the table. Travel is set
    out trip by trip or conference by conference, each with an
    estimated amount, also for conferences you are only considering. A
    trip or conference belongs in the budget of the year it takes
    place, and for a trip, stay or service you ask no money for, say
    who pays.
  - Later years: the outlook is entered with overhead included and can
    be raised at continuation by at most 10 percent. Explain a
    significant rise.
  - Other projects: list every project you and your leader take part
    in at the time of applying, from any funder, and compare the field
    with both CVs.
  - Planned results: a finished project is assessed on original
    publications accepted for print (or a patent), against what the
    proposal planned. Choose a duration in which that is realistic.
